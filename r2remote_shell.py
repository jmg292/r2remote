...
class ShellHandler(MessageHandlerMixin):
    ...
    def __init__(self, identity_key, authorized_hosts):
    ...
    def run(self):
        ...
        while command != "exit":
            ...
            if command:
                for packed_message in Message.packed_from_string(self.message_wrapper, command):
                    self._write_line(packed_message)
                if command != "exit":                    
                    response = self._get_message(self.message_wrapper)
                    try:
...
if __name__ == "__main__":
    ...
    shell = ShellHandler(arguments.identity_key, arguments.authorized_hosts)
    print("Done.\n[+] Connecting . . . ")
